/*
 * Tarea TSP-MPI - Sistemas Distribuidos 2014-1
 *
 * Felipe Astroza
 * Franzel Madariaga
 *
...
void masterWork(int *matrix, int n)
{
    ...
    // Esto agrega las tareas iniciales
    for (i = 0; i < n - 1; i++) {
	task = newTask(1, n);
    ...
    do {
	DEBUG("availableWorkers: %d, taskBag: %d",
	      availableWorkers->itemsCount, taskBag->itemsCount);
	DEBUG("[Master] Esperando mensajes..");
	MPI_Probe(MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
	DEBUG("[Master] Nuevo mensaje de rank=%d con tag=%d",
	...
	if (status.MPI_TAG == TASK_TAG) {
	    ...
	    DEBUG("[Master] Mensaje con %d tareas", elementsCount);
	    for (i = 0; i < elementsCount; i++) {
		task = calc_task_pointer(tasks, n, i);

		// Recorrio el camino hasta el inicio?
	    ...
	    free(tasks);
	    // Intenta asignar tareas
	    while (availableWorkers->itemsCount > 0
		   && taskBag->itemsCount > 0) {
		task = listDequeue(taskBag);
		rank = (int) listDequeue(availableWorkers);
		task->longMin = longMin;
		MPI_Send(task, 1, taskType, rank, TASK_TAG,
			 MPI_COMM_WORLD);
		free(task);
	    }
	} else {
	    // AVAILABLE_TAG
	    MPI_Recv(&rank, 1, MPI_INT, status.MPI_SOURCE, status.MPI_TAG,
...
void findCitiesToVisit(int *visited, int visitedCount, int *ret, int n)
{
	...
	DEBUG("findCitiesToVisit: visited[i=%d]=%d", i, visited[i]);
...
void slaveWork(int *matrix, int n, int rank)
{
    MPI_Datatype taskType = constructWorkType(n);
	...
	MPI_Recv(task, 1, taskType, 0, TASK_TAG, MPI_COMM_WORLD,
		 MPI_STATUS_IGNORE);
	DEBUG("Tarea recibida de rank=%d", rank);
	if (task->visitedCount >= n) {
	    // Ya visito todas, falta volver a 0
	    task->partialLong +=
		matrix[task->visited[task->visitedCount - 1] * n];
	    task->visitedCount++;
	    // Poda: si partialLong supera o es igual a longMin, no se envia de vuelta
		...
		if (citiesToVisit > 0) {
		    ...
		    findCitiesToVisit(task->visited, task->visitedCount,
				      cities, n);
		    for (i = 0; i < citiesToVisit; i++) {
			t = calc_task_pointer(tasks, n, i);
			memcpy(t, task,
			       sizeof(TSP_task) + sizeof(int) * n);
			t->cityToVisit = cities[i];
		    }
		    MPI_Send(tasks, citiesToVisit, taskType, 0, TASK_TAG,
			     MPI_COMM_WORLD);
		    free(tasks);
		} else {
		    MPI_Send(task, 1, taskType, 0, TASK_TAG,
			     MPI_COMM_WORLD);
		}
	    }
	}
